...
class ModelTestCase(TestCase):
    ...
    def test_get_war_result_less(self):
        card = Card.objects.create(suit=Card.DIAMOND, rank='five')
        card2 = Card.objects.create(suit=Card.DIAMOND, rank='six')
        self.assertEqual(card.get_war_result(card2), -1)

    def test_get_war_result_equal(self):
        card = Card.objects.create(suit=Card.DIAMOND, rank='five')
        card2 = Card.objects.create(suit=Card.DIAMOND, rank='two')
        self.assertEqual(card.get_war_result(card2), 1)

    # LESS REPEATED LINES - dry testing
    def setUp(self):
        self.card = Card.objects.create(suit=Card.CLUB, rank='jack')

    def test_get_ranking2(self):
